...
type KotlinCompileData struct {
	target           android.ModuleOutPath
	pcStateFileNew   android.OutputPath
	pcStateFilePrior android.OutputPath
	diffFile         android.OutputPath
}

var inputDeltaRule = pctx.AndroidRemoteStaticRule("input-delta", android.RemoteRuleSupports{Goma: true},
	blueprint.RuleParams{
		Command: `${config.FindInputDeltaCmd} --target "$target" --inputs_file "$target.rsp" --new_state "$newStateFile" --prior_state "$priorStateFile" > $out`,
		CommandDeps: []string{
	...
	"target", "newStateFile", "priorStateFile")
...
const MoveDeltaStateFile = `mv $inputDeltaFileNew $inputDeltaFileOld `

var kotlinc = pctx.AndroidRemoteStaticRule("kotlinc", android.RemoteRuleSupports{Goma: true},
	blueprint.RuleParams{
		Command: NonIncKotlinCmd + ` && ` + MoveDeltaStateFile,
		CommandDeps: []string{
			"${config.KotlincCmd}",
	...
	"inputDeltaFileNew", "inputDeltaFileOld", "name")
...
var kotlinIncremental = pctx.AndroidRemoteStaticRule("kotlin-incremental", android.RemoteRuleSupports{Goma: false},
	...
	"classesDir", "headerClassesDir", "headerJar", "kotlinJvmTarget", "kotlinBuildFile", "emptyDir",
	"name", "outputDir", "buildDir", "workDir", "inputDeltaFileNew", "inputDeltaFileOld", "sourceDeltaFile")
...
// kotlin can compile incrementally if it has a list of sources that have been added, modified, or
// deleted. This rule calculates that, for use by both main kotlin compilation and kapt.
func (j *Module) kotlinInputDelta(ctx android.ModuleContext, data KotlinCompileData, srcFiles android.Paths, commonSrcFiles android.Paths) {

	var srcs android.Paths
	srcs = append(srcs, srcFiles...)
	srcs = append(srcs, commonSrcFiles...)

	ctx.Build(pctx, android.BuildParams{
		Rule:            inputDeltaRule,
		Description:     "Calculate source changes for kotlin",
		Inputs:          srcs,
		Output:          data.diffFile,
		ImplicitOutputs: []android.WritablePath{data.pcStateFileNew},
		Args: map[string]string{
			"target":         data.target.String(),
			"newStateFile":   data.pcStateFileNew.String(),
			"priorStateFile": data.pcStateFilePrior.String(),
		},
	})
}
...
func (j *Module) kotlinCompile(ctx android.ModuleContext, outputFile, headerOutputFile android.WritablePath,
	...
	// Creates a dependency on the `input-delta` rule.
	deps = append(deps, compileData.diffFile)
	...
	args := map[string]string{
		...
		"name":               kotlinName,
		"inputDeltaFileNew":  compileData.pcStateFileNew.String(),
		"inputDeltaFileOld":  compileData.pcStateFilePrior.String(),
