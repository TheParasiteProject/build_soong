...
func TestApkCertsOverrideApps(t *testing.T) {
	result := android.GroupFixturePreparers(
		fixture,
		android.PrepareForNativeBridgeEnabled,
		...
		android_device {
			name: "mydevice",
			system_partition_name: "myfilesystem",
		}
		android_filesystem {
			name: "myfilesystem",
			compile_multilib: "both",
			deps: ["app", "override_app"],
		}
		android_app {
			name: "app",
			platform_apis: true,
	...
	apkCerts := result.ModuleForTests(t, "mydevice", "android_x86_64_silvermont").Output("apkcerts_without_app_sets.txt")
	content := android.ContentFromFileRuleForTests(t, result.TestContext, apkCerts)
	expected := `name="override_app.apk" certificate="build/make/target/product/security/testkey.x509.pem" private_key="build/make/target/product/security/testkey.pk8" partition="system"` + "\n"
	android.AssertStringEquals(t, "apkcerts.txt contains override apps info", expected, content)
}
